...
$(document).ready(function () {
    ...
    if ($('#info-body').length) {
        navTl.to('#info-body', {
            transformOrigin: 'bottom center', scaleY: 1, duration: 0.5, ease: 'power1.out',
            onStart: () => {
                $('.info-nav').addClass('text-black').removeClass('invert-text bottom-0')
                $('.menu-nav').addClass('hidden')
                $('.info-nav>button').text('close')
            },
            onReverseComplete: () => {
                $('.info-nav').removeClass('text-black').addClass('invert-text')
                $('.menu-nav').removeClass('hidden')
                $('.info-nav>button').text('info')
            }
        }).to('.info-nav', {bottom: null, top: 0, duration: 0.5}, '=')
            .from($('#info-body').find('.body-item'), {opacity: 0, y: 50, duration: 0.5, stagger: 0.3}, '<=0.3');
    ...
    menuTl
        .to('.menu-body', {
            transformOrigin: 'top center', scaleY: 1, duration: 0.5, ease: 'power1.out',
            onStart: () => {
        ...
        .from('.titles', {opacity: 0, y: -50, duration: 0.5, stagger: 0.3}, '<=0.3');
